...
public class StringTable {
    ...
    {  
    	int hash_code = stringToHashCode(r.key);
    	int index = toIndex(hash_code);
    	
    	for (int i=0; i<buckets[index].size(); i++) {
    		if (r.key.equals(buckets[index].get(i).key)) {
    			return false;
    		}
    	}
    ...
    {
    	int hash_code = stringToHashCode(key);
    	int index = toIndex(hash_code);
    ...
    private int toIndex(int hashcode)
    {
    	...
    	int hashcode_abs = Math.abs(hashcode);
    	int index = (int)Math.floor(((hashcode_abs * A)%1.0) * nBuckets);
