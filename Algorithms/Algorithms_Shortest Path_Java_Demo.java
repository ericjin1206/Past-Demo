...
public class ShortestPaths {
    ...
    public void run() {
    	...
    	while (pq.isEmpty() == false) {
    		PQEntry<VertexAndDist, Integer> curMin = pq.extractMin();
    		
    		Vertex curVer = (curMin.getElement()).getVertex();
    		
    		for (Edge e: curVer.edgesFrom()) {
    			PQEntry<VertexAndDist, Integer> nextHandle = handles.get(e.to);
    			Vertex nextVertex = (nextHandle.getElement()).getVertex();
    			int nextDist = (nextHandle.getElement()).getDistance();
    			int curDist = (curMin.getElement()).getDistance() + weights.get(e);
    			
    			if (curDist < nextDist) {
    				(nextHandle.getElement()).setDistance(curDist);
    				nextHandle.updatePriority(curDist);
    				parentEdges.put(nextVertex, e);
    ...
    public LinkedList<Edge> returnPath(Vertex endVertex) {
    	...
    	//
    	Vertex curVer = endVertex;
    	while (curVer.equals(startVertex) == false){
    		Edge e = parentEdges.get(curVer);
    		curVer = e.from;
    		path.addFirst(e);
